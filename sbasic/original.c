 ...
 *   I found the book on [the Internet Archive](https://archive.org/details/cpowerusersguide00schi_0/mode/2up).
 */
...
void scan_labels()
{
	...
	do {
		...
		if(token_type==NUMBER) {
			...
			label_table[addr].p = prog; /* current point in the program */
		}
...
/* Return index of next free position in label array.
   ...
   A -2 is returned when a duplicate label is found.
*/
get_next_label(s)
char *s;
{
...
void exec_for()
{
	...
	else /* otherwise skip the loop altogether */
...
void next()
{
	...
	variables[i.var]++; /* increment the control variable */
...
get_token()
{
	...
	while(iswhite(*prog)) ++prog;  /* skipover white space */
